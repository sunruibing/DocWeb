...
public class FindCyclopediaList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			Integer pageCount = Integer
					.parseInt(request.getParameter("pageCount"));
			List<Cyclopedia> cycloperdiaList = new CyclopediaService()
					.queryCylopediaList(pageCount);
			PrintWriter wr = response.getWriter();
			wr.print(new Gson().toJson(cycloperdiaList));// success
			wr.flush();
		} catch (NumberFormatException e) {
			e.printStackTrace();
			response.getWriter().println(2);// NumberFormat error
	...
	public void destroy() {
		...
		try {
			DriverManager
					.deregisterDriver(DriverManager.getDrivers().nextElement());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
