...
public class CyclopediaService {
	public List<Cyclopedia> queryCylopediaList(Integer PageCount)
			throws SQLException {
		String sql = " SELECT id,icon,cover,title,content,time, collect_count FROM cyclopedia order by id desc limit "
				+ PageCount + ",20; ";
		...
		try {
			...
			while (result.next()) {
				...
				cyclopedia.setContent(content.substring(0,
	...
	public Cyclopedia queryCyclopediaById(Integer cyclopediaId)
			throws SQLException {
		String sql = "SELECT cover,title,content,time FROM cyclopedia WHERE id = "
				+ cyclopediaId + " ";
		DBUtil dbUtil = new DBUtil(sql);
		Cyclopedia cyclopedia = new Cyclopedia();
		ResultSet result = null;
	...
	public void insertCollectCount(Integer count, Integer cyclopediaId) {
		String sql = " update cyclopedia set collect_count = " + count
				+ " where id = " + cyclopediaId + " ";
		DBUtil dbUtil = new DBUtil(sql);
		try {
	...
	public int insertCyclopedia(Cyclopedia cyclopedia) {
		String sql = " insert into cyclopedia(icon, cover, title, content, time) values("
				+ "'" + cyclopedia.getIcon() + "'" + "," + "'"
				+ cyclopedia.getCover() + "'" + "," + "'"
				+ cyclopedia.getTitle() + "'" + "," + "'"
				+ cyclopedia.getContent() + "'" + "," + "'"
				+ cyclopedia.getTime() + "'" + ")";
		DBUtil dbUtil = new DBUtil(sql);
		try {
			dbUtil.pst.executeUpdate();
	...
	public List<Map<String, Object>> findCyclopediaById(Integer id)
			throws SQLException {
		String sql = "select id,icon,title,content,cover from cyclopedia where id = "
				+ id + "";
		DBUtil dbUtil = new DBUtil(sql);
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		ResultSet result = null;
	...
	public int updataCyclopedia(Integer id, String icon, String title,
			String content, String cover) {
		String sql = " UPDATE cyclopedia SET icon=" + "'" + icon + "'"
